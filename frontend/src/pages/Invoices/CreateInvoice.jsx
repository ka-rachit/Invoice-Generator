...
import {bumpInsightVersion} from "../../utils/insightVersion";
...
const CreateInvoice = ({existingInvoice, onSave}) => {
    ...
    useEffect(() => {
        ...
        if (aiData) {
            setFormData((prev) => ({
                ...
                items: aiData.items || [{name: "", quantity: "", unitPrice: ""}],
            }));
        }
    ...
    const handleAddItem = () => {
        setFormData({...formData, items: [...(formData.items || []), {name: "", quantity: "", unitPrice: ""}]});
    };
    ...
    const inputClasses = "w-full h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent";
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 pb-[100vh]">
            ...
            <div className="flex items-center gap-4 p-4 sm:p-6 border-t border-slate-200">
                <Button type="button" variant="secondary" onClick={handleAddItem} icon={Plus}>
                    Add Item
                </Button>
                <div className="flex items-center gap-2 ml-auto">
                    <label className="text-sm font-medium text-slate-700 whitespace-nowrap">Tax (%)</label>
                    <input
                        type="number"
                        name="taxPercent"
                        value={formData.taxPercent}
                        onChange={handleInputChange}
                        className="w-24 h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="0"
                        min="0"
                    />
                </div>
